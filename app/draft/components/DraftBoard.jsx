...
export default function DraftBoard({ order, picks, config, myTeam }) {
  ...
  return (
    <div className="retro-panel p-2 sm:p-4 mb-4 overflow-x-auto">
      <h2 className="text-[9px] sm:text-[10px] text-[#00bcd4] mb-3 text-center">
        DRAFT BOARD
      </h2>
      ...
      <div className="min-w-[700px]">
        ...
        <div className="grid gap-[2px]" style={{ gridTemplateColumns: `40px repeat(${TOTAL_TEAMS}, 1fr)` }}>
          <div className="text-[6px] text-gray-500 p-1" />
          {order.map((team, i) => (
            <div
              key={team.token || i}
              className={`text-[6px] sm:text-[7px] p-1 text-center truncate rounded-t ${
        ...
        {Array.from({ length: TOTAL_ROUNDS }, (_, roundIdx) => {
          ...
          return (
            ...
            >
              {/* Round label */}
              <div className="flex items-center justify-center text-[7px] text-gray-500 p-1">
                <span>R{round}</span>
              ...
              {Array.from({ length: TOTAL_TEAMS }, (_, colIdx) => {
                ...
                return (
                  <div
                    key={colIdx}
                    className={`draft-cell p-1 rounded text-center min-h-[36px] sm:min-h-[42px] flex flex-col items-center justify-center transition-all ${
                      isCurrentPick
                  ...
                  >
                    {pick ? (
                      <>
                        <span className="text-[6px] sm:text-[7px] text-[#e8d5a3] leading-tight truncate w-full">
                          {pick.playerName?.split(" ").pop()}
                        </span>
                        <span className="text-[5px] sm:text-[6px] text-gray-400 truncate w-full">
                          {pick.playerTeam?.slice(0, 10)}
                        </span>
                        {pick.isAutoPick && (
                          <span className="text-[5px] text-purple-400">AUTO</span>
                        )}
                      </>
                    ) : (
      ...
      <div className="flex items-center justify-center gap-4 mt-3 flex-wrap">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[6px] text-gray-400">PICKED</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="w-3 h-3 bg-[#2a1a3a] border border-purple-500/30 rounded" />
          <span className="text-[6px] text-gray-400">AUTO-PICK</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="w-3 h-3 border border-[#f5c542] rounded on-the-clock" />
          <span className="text-[6px] text-gray-400">ON CLOCK</span>
        </div>
      </div>
    </div>
  );
